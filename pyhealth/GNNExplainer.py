...
from torch_geometric.explain import ExplainerConfig, Explanation, HeteroExplanation, ModelConfig
...
class GNNExplainer(ExplainerAlgorithm):
    ...
    def forward(
        ...
        **kwargs,
        ...
        node_mask = self._extract_mask(self.node_mask, self.hard_node_mask, is_hetero)
        edge_mask = self._extract_mask(self.edge_mask, self.hard_edge_mask, is_hetero)
    ...
    def _extract_mask(self, mask, hard_mask, is_hetero: bool):
        if is_hetero:
            return {key: self._post_process_mask(mask[key], hard_mask[key], apply_sigmoid=True)
                    for key in mask}
        else:
    ...
    def _update_hard_masks(self, is_hetero: bool):
        if is_hetero:
            for key in self.node_mask:
                self.hard_node_mask[key] = self.node_mask[key].grad != 0.0 if self.node_mask[key].grad is not None else None
            for key in self.edge_mask:
                self.hard_edge_mask[key] = self.edge_mask[key].grad != 0.0 if self.edge_mask[key].grad is not None else None
        else:
            self.hard_node_mask = self.node_mask.grad != 0.0 if self.node_mask is not None and self.node_mask.grad is not None else None
            self.hard_edge_mask = self.edge_mask.grad != 0.0 if self.edge_mask is not None and self.edge_mask.grad is not None else None
