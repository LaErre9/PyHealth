    ...
    def _compute_disconnection_penalty(self) -> torch.Tensor:
        ...
        for key, mask in self.edge_mask.items():
            ...
            if selected_edges.size(1) == 0:
                ...
                # Nessun spigolo selezionato, quindi nessuna penalità
                continue

            # Costruzione del grafo e calcolo delle componenti connesse
            G = nx.from_edgelist(selected_edges.t().tolist())
            if len(G) == 0:
                print("Empty graph")
                # Grafo vuoto, quindi nessuna penalità
                continue
