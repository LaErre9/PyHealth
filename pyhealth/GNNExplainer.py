    ...
    def _loss(self, y_hat: Tensor, y: Tensor) -> Tensor:
        ...
        if isinstance(self.edge_mask, dict):
            ...
            if self.hard_node_mask is not None:
                ...
                loss = loss + self.coeffs['node_feat_ent'] * ent.mean()
        print('Loss: ' + str(loss))
        # # Regolarizzazione L1 per la sparsità
        # l1_penalty = self.coeffs['l1_penalty']  # Aggiungi un valore di default per il termine L1
 
 
        # if isinstance(self.edge_mask, dict):
        #     # Grafici eterogenei
        #     for key in self.edge_mask:
        #         loss += l1_penalty * self.edge_mask[key].abs().sum()
        #     for key in self.node_mask:
        #         loss += l1_penalty * self.node_mask[key].abs().sum()
        # else:
        #     # Grafici omogenei
        #     if self.edge_mask is not None:
        #         loss += l1_penalty * self.edge_mask.abs().sum()
        #     if self.node_mask is not None:
        #         loss += l1_penalty * self.node_mask.abs().sum()
